...
import {
  Book, Rocket, Upload, Mic, CheckCircle2, Loader2, Radio,
  ImagePlus, Send, X, ChevronUp, MoreVertical, FileText, LogOut
} from "lucide-react";
...
const STT_LANGUAGES = AVAILABLE_LANGS.map(l => ({
  code: l.sttCode,
  label: l.label
}));
...
1️⃣ Student is asking about a word (any form):
   - Direct question: "Was bedeutet X?" / "What is X?" / "ما معنى X؟"
   - Indirect: "I don't know that word" / "ما فهمت هالكلمة"
   - Confirmation: "Does X mean...?" / "هل X يعني...؟"
   → They are trying to learn the word — TAG IT

2️⃣ Student used the WRONG word and you corrected them:
   - Their answer was incorrect, you taught them the right word
   → They learned a new correct word — TAG IT

3️⃣ Student couldn't say the word in ${targetLabel}:
   - They used ${motherLabel} instead because they didn't know it
   - You taught them the ${targetLabel} equivalent
   → They learned how to say it — TAG IT

4️⃣ YOU actively taught them a new word:
   - You explained a word's meaning as part of teaching
   - You translated a word to help them understand
   - NOT basic everyday words they clearly already know
   → You taught it, they learned it — TAG IT

## Key insight:
If explaining/teaching this word was a LEARNING MOMENT → TAG IT
If you just used the word casually in conversation → DON'T TAG IT
...
Once you write [VOCAB:...], you are DONE.
❌ Do NOT repeat the word
❌ Do NOT say "word = translation" again
❌ The tag is invisible to the student — treat it like it doesn't exist

## EXAMPLE ✅:
Student: "Was bedeutet 'erzählen'?"
Teacher: "'Erzählen' يعني 'يحكي'. Kannst du einen Satz sagen?"
[VOCAB: erzählen=يحكي]

Student: "ما فهمت كلمة Krankenhaus"
Teacher: "Krankenhaus يعني مستشفى. Wiederhole bitte!"
[VOCAB: Krankenhaus=مستشفى]

## REMINDER:
Before sending your response, ask yourself:
"Did the student just learn a new word?"
If YES → make sure [VOCAB:...] is at the very end.
If NO → send without tag.
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedTarget && availableCodes.includes(savedTarget)) setTargetLang(savedTarget);
      if (savedMother && availableCodes.includes(savedMother)) setMotherLang(savedMother);
  ...
  useEffect(() => {
    ...
    const authUnsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      ...
      if (firebaseUser) {
        const flashcardsRef2 = collection(db, "users", firebaseUser.uid, "flashcards");
        const q = query(flashcardsRef2, orderBy("createdAt", "desc"));

        flashcardsUnsubscribe = onSnapshot(q, (snapshot) => {
          const cards = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
          flashcardsRef.current = cards;
          setFlashcards(cards);
          console.log(`🔄 [FLASHCARDS SYNC] ${cards.length} cards loaded`);
        });

      } else {
        flashcardsRef.current = [];
        setFlashcards([]);
        if (flashcardsUnsubscribe) {
          flashcardsUnsubscribe();
          flashcardsUnsubscribe = null;
        }
      }
    });
    ...
    return () => {
      authUnsubscribe();
      if (flashcardsUnsubscribe) flashcardsUnsubscribe();
    };
  }, []);
  ...
  Messages: ${sentMessagesCountRef.current}

  Review the ENTIRE conversation above and provide a detailed, honest summary following this EXACT format:
  ...
  ⚠️ What needs work:
  - [Specific mistake 1 with exact quote]
  - [Specific mistake 2 with exact quote]
  ...
  RULES:
  - Be BRUTALLY HONEST — no false praise
  - Quote EXACT mistakes from the conversation
  - Give ACTIONABLE next steps
  - Keep it under 150 words
  - Use this EXACT format with emojis
  - Write EVERYTHING in ${targetLabel}
    `.trim();
  }
  ...
  const handleViewReport = () => {
    try {
      ...
      liveClientRef.current?.sendText(summaryPrompt);
      setMessages(prev => [
        ...prev,
        { role: "student", text: "📊 Generate session summary" },
        { role: "teacher", text: "...", isSummary: true }
      ]);
    } catch (e) {
      console.error("❌ [VIEW REPORT]", e);
    }
  };
  ...
  const handleEndSessionConfirm = async () => {
    try {
      ...
      console.error("❌ [END SESSION CONFIRM]", e);
    }
  };
  ...
  function processJson(json) {
    try {
      ...
      if (json.setupComplete) {
        ...
        if (selectedImageRef.current && liveClientRef.current) {
          setTimeout(() => {
            try {
              liveClientRef.current?.sendImage({
                base64:   selectedImageRef.current.base64,
                mimeType: selectedImageRef.current.mimeType,
              });
            } catch (_) {}
          }, 500);
        }
      ...
      if (serverContent.outputTranscription?.text) {
        ...
        const cleanText = rawText
          .replace(/^Transkript:\s*/i, "")
          .replace(/\[VOCAB:[^\]]*\]/gi, "");
        teacherBufferRef.current += cleanText;
        setMessages(prev => {
          const last = prev[prev.length - 1];
          if (last?.role === "teacher") {
            return [...prev.slice(0, -1), { ...last, text: teacherBufferRef.current }];
          }
    ...
    } catch (e) { console.error("❌ [PROCESS JSON ERROR]", e); }
  }
  ...
  async function handleIncomingData(data) {
    try {
      ...
      playRawAudio(data);
    } catch (e) { console.error("❌ [INCOMING ERROR]", e); }
  }
  ...
  function handleReconnect() {
    try {
      ...
      const systemWithHistory = enrichedSystemRef.current
        ? `${enrichedSystemRef.current}\n\n## Previous Conversation:\n${chatSummary}`
        : enrichedSystemRef.current;
      if (liveClientRef.current && systemWithHistory) liveClientRef.current.updateSystem(systemWithHistory);
      teacherBufferRef.current         = "";
      studentBufferRef.current         = "";
      studentVoiceMsgActiveRef.current = false;
      messageCountRef.current          = 0;
      ...
      setIsLive(false);
      setMessages(prev => {
        chatHistoryRef.current = prev;
        return [...prev, { role: "teacher", text: tr("messages", "reconnecting") }];
      });
  ...
  async function startLiveMic() {
    try {
      ...
      console.error("❌ [LIVE MIC START]", err);
      toast.error("Microphone Error: " + err.message);
    }
  }

  async function toggleLive() {
    if (isLive) { await stopLiveMic(); return; }
    await startLiveMic();
  }
  ...
  const handleUploadFile = async (fileObj) => {
    ...
    try {
      ...
      setReferenceFiles(prev => prev.map(f => f.id === fileObj.id
        ? { ...f, status: "success", uri: result.fileUri, mimeType: result.mimeType, fileType: result.type }
        : f
      ));
    } catch (err) {
  ...
  const handleFileChange = (e) => {
    try {
      if (!apiKey) {
        toast.error(tr("setup", "apiKeyPlaceholder"));
        return;
      }
      ...
      console.error("❌ [FILE CHANGE]", e);
      toast.error("Error selecting file: " + e.message);
    }
  };
  ...
  const handleStartNoFiles = async () => {
    try {
      ...
      console.error("❌ [START NO FILES]", e);
      toast.error("Error starting session: " + e.message);
    }
  };
  ...
  const handleStart = async () => {
    try {
      ...
      if (requirement === "required" && successFiles.length === 0) {
        toast.warning(tr("smartButton", "required"));
        return;
      }
  ...
  const handleSaveLang = () => {
    setTargetLang(tmpTarget);
    setMotherLang(tmpMother);
    setLangMode(tmpMode);
    try {
      localStorage.setItem("targetLang", tmpTarget);
      localStorage.setItem("motherLang", tmpMother);
      localStorage.setItem("langMode",   tmpMode);
  ...
  const filteredSttLangs = STT_LANGUAGES.filter(
    l => l.code.startsWith(targetLang) || l.code.startsWith(motherLang)
  );
  ...
  const handleGoogleLogin = async () => {
    ...
    if (firebaseUser) {
      console.log("✅ [LOGIN] Signed in:", firebaseUser.displayName);
    }
  };
  ...
  const parseAndSaveVocab = async (text) => {
    if (!user) return;
    console.log("🎤 [AI MESSAGE]", text.substring(0, 200) + "...");
    const match = text.match(/\[VOCAB:\s*([^\]]+)\]/i);
    if (!match) {
      console.log("❌ [NO VOCAB TAG] AI didn't add any [VOCAB:...] tag");
      return;
    }
    console.log("✅ [VOCAB TAG FOUND]", match[0]);
    const pairs = match[1].split("|").map(p => p.trim()).filter(Boolean);
    console.log("📝 [VOCAB PAIRS]", pairs);

    for (const pair of pairs) {
      const parts = pair.split("=").map(s => s.trim());
      if (parts.length !== 2) {
        console.warn("⚠️ [VOCAB] Invalid format, skipping:", pair);
        continue;
      }
      const [word, translation] = parts;
      if (!word || !translation) {
        console.warn("⚠️ [VOCAB] Empty word or translation, skipping");
        continue;
      }
      const exists = flashcardsRef.current.some(f =>
        f.word.toLowerCase() === word.toLowerCase() &&
        f.targetLang === targetLang &&
        f.motherLang === motherLang
      );
      if (!exists) {
        console.log(`💾 [VOCAB SAVING] ${word} = ${translation}`);
        const result = await saveFlashcard(user.uid, word, translation, targetLang, motherLang);
        if (!result?.success) {
          console.error(`❌ [SAVE FAILED]`, result?.error);
          if (result?.error === "invalid_format" || result?.error === "invalid_word") {
            console.warn("⚠️ [VOCAB] Skipped invalid word:", word);
          } else {
            toast.error(`Failed to save "${word}" to flashcards`);
          }
        } else {
          console.log(`✅ [VOCAB SAVED SUCCESS] ${word}`);
        }
      } else {
        console.log(`⏭️ [VOCAB EXISTS] ${word} already in flashcards`);
      }
    }
  };
  ...
  return (
    <main
      className={`fixed inset-0 flex flex-col w-full
        ...
        ${isDarkMode ? "bg-zinc-900" : "bg-[#f8fafc]"}
        p-4 sm:p-6 lg:p-8
      `}
    >
      <button
      ...
      <AnimatePresence mode="wait">
        {step === "setup" ? (
          <motion.div
            ...
            className="flex flex-col items-stretch w-full max-w-md h-full min-h-0"
          >
            {/* Header: Roboter & Titel */}
            <div className="flex flex-col items-center shrink-0 mb-[clamp(0.5rem,2vh,1.5rem)]">
              <div className="w-[clamp(60px,10vh,88px)] h-[clamp(60px,10vh,88px)] transition-all duration-300">
                <RobotSVG size="100%" />
              </div>
              <h1
                className={`setup-title text-[clamp(1.5rem,4vh,2.25rem)] font-black leading-none tracking-tighter text-center transition-colors px-2
            ...
            {/* Haupt-Setup-Karte */}
            <div
              className={`setup-card w-full flex flex-col
                flex-1 min-h-0
                overflow-hidden rounded-[2.5rem]
                p-[clamp(1rem,3vh,2rem)]
                gap-[clamp(0.5rem,1.5vh,1rem)]
                shadow-2xl border transition-all duration-300
                ${isDarkMode ? "bg-zinc-800 border-zinc-700" : "bg-white border-white"}
              `}
            >
              {/* Auth/Signin Bar */}
              <div className={`w-full flex items-center justify-between rounded-2xl border transition-all shrink-0 px-4 h-[clamp(40px,6vh,52px)] ${
                  isDarkMode ? "border-zinc-700 bg-zinc-900/50" : "border-zinc-200 bg-zinc-50"
                }`}>
                {user ? (
                  <div className="flex items-center justify-between w-full min-w-0">
                    <div className="flex items-center gap-2 min-w-0">
                      <img src={user.photoURL} alt="avatar" className="w-6 h-6 rounded-full shrink-0 object-cover" />
                      <span className={`text-xs font-bold truncate ${isDarkMode ? "text-zinc-300" : "text-zinc-700"}`}>
                        {user.displayName}
                      </span>
                    </div>
                    <button onClick={handleSignOut} className="text-xs text-red-400 hover:text-red-600 cursor-pointer font-bold shrink-0 ml-2">
                      Sign out
                    </button>
                  </div>
                ) : (
                  <button onClick={handleGoogleLogin} className="w-full h-full flex items-center justify-center gap-2 cursor-pointer">
                    ...
                    <span className={`text-sm font-bold ${isDarkMode ? "text-zinc-300" : "text-zinc-600"}`}>
                      Sign in with Google
                    </span>
                  </button>
                )}
              ...
              <div className="flex flex-col gap-1 shrink-0">
                <input
                  ...
                  className={`w-full rounded-2xl border px-4 outline-none text-sm transition-all h-[clamp(40px,6vh,52px)] ${
                    referenceFiles.length > 0
                      ? (isDarkMode ? "border-zinc-700 bg-zinc-800 text-zinc-500 cursor-not-allowed" : "border-zinc-100 bg-zinc-50 text-zinc-400 cursor-not-allowed")
                      : (isDarkMode ? "border-zinc-600 bg-zinc-700 text-white focus:border-sky-400" : "border-zinc-100 bg-white text-zinc-800 focus:border-sky-400")
                  }`}
                />
                <p className={`text-[10px] truncate leading-none px-2 ${isDarkMode ? "text-zinc-500" : "text-zinc-400"}`}>
              ...
              <div className="shrink-0">
                <button
                  ...
                  className={`w-full flex items-center justify-between rounded-2xl border-2 px-4 hover:border-sky-300 transition-all cursor-pointer text-sm font-medium group h-[clamp(40px,6vh,52px)] ${
                    isDarkMode ? "border-zinc-600 text-zinc-300 hover:bg-sky-900/30 hover:border-sky-500" : "border-zinc-100 text-zinc-600 hover:bg-sky-50 hover:border-sky-300"
                  }`}
                >
                  ...
                  <span className={`text-[10px] font-normal truncate ml-2 text-right ${isDarkMode ? "text-zinc-400 group-hover:text-sky-300" : "text-zinc-400 group-hover:text-sky-600"}`}>
              ...
              <div className="shrink-0">
                ...
                <div className={`text-[10px] pl-2 pt-1 leading-tight ${isDarkMode ? "text-zinc-500" : "text-zinc-400"}`}>
              ...
              <div className="space-y-1.5 shrink-0">
                <label className={`font-bold flex items-center gap-2 text-sm pl-1 mb-0.5 ${isDarkMode ? "text-zinc-300" : "text-zinc-700"}`}>
                  <Book size={14} className="text-emerald-500" /> {tr("setup", "filesLabel")}
                </label>
                <label className={`relative flex items-center justify-center gap-3 rounded-2xl border-2 border-dashed cursor-pointer transition-all h-[clamp(40px,6vh,52px)] ${
                    isDarkMode ? "bg-sky-900/10 border-sky-700/50 hover:bg-sky-900/20" : "bg-sky-50/30 border-sky-200 hover:bg-sky-50"
                  }`}>
              ...
              {/* Dateiliste - Der flexible, scrollbare Bereich */}
                ...
                <div className="space-y-1.5">
                  {referenceFiles.map(f => (
                    <div key={f.id} className={`file-item flex items-center justify-between h-8 px-3 rounded-xl border text-xs leading-none font-medium transition-colors ${
                      isDarkMode ? "bg-zinc-700/50 border-zinc-600 text-zinc-300" : "bg-zinc-50 border-zinc-100 text-zinc-700"
                    }`}>
              ...
              <div className="shrink-0 pt-1">
                <button
                  onClick={handleStart}
                  disabled={referenceFiles.some(f => f.status === "uploading") || isAnalyzing || smartButton.disabled}
                  className={`start-button w-full font-black rounded-[1.8rem] shadow-xl transition-all flex items-center justify-center gap-3 text-lg active:scale-95 hover:scale-[1.02] h-[clamp(50px,8vh,64px)] ${
                    smartButton.disabled ? "bg-zinc-400 text-zinc-100 cursor-not-allowed" : "bg-sky-500 hover:bg-sky-600 text-white"
                  }`}
                >
                  {isAnalyzing
                    ? <><Loader2 size={20} className="animate-spin" /> {tr("setup", "analyzingBtn")}</>
                    : <><Rocket size={20} /> {smartButton.text}</>
                  }
                </button>
              </div>
            </div>
          ...
          >
            <div className={`flex items-center justify-between px-4 lg:px-6 py-2 lg:py-3 border-b shrink-0 transition-colors ${
              isDarkMode ? "border-zinc-700 bg-zinc-800" : "border-zinc-100 bg-white"
            }`}>
              <span className={`text-xs lg:text-sm font-medium ${isDarkMode ? "text-zinc-500" : "text-zinc-300"}`}>
                {tr("classroom", "headerTitle")}
              </span>
              <span className={`text-xs lg:text-sm font-medium ${isDarkMode ? "text-zinc-500" : "text-zinc-300"}`}>
            ...
            <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
              <div className={`flex flex-col flex-1 overflow-hidden border-r transition-colors ${
                isDarkMode ? "border-zinc-700" : "border-zinc-100"
              }`}>
                <div className={`flex-1 overflow-y-auto overscroll-y-contain p-6 lg:p-8 space-y-4 lg:space-y-5 transition-colors ${
                  isDarkMode ? "bg-zinc-900/50 text-zinc-100" : "bg-zinc-50/50 text-zinc-800"
                }`}>
                  {messages.map((m, i) => (
                    <div key={i} className={`flex ${m.role === "student" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[80%] rounded-[1.5rem] shadow-sm overflow-hidden text-sm lg:text-base ${
                        m.role === "student"
                          ...
                          : m.isSummary
                            ? isDarkMode
                              ? "bg-gradient-to-br from-emerald-900/50 to-emerald-800/50 border-2 border-emerald-600 text-zinc-100"
                        ...
                        {m.isImage && m.imageUrl && (
                          <div className="relative">
                            ...
                            <div className="absolute bottom-3 right-3 bg-emerald-500 rounded-full p-0.5">
                              <CheckCircle2 size={10} className="text-white" />
                            </div>
                          </div>
                        )}
                        <p className={`leading-relaxed px-4 pb-3 pt-1 whitespace-pre-wrap ${m.isSummary ? "font-medium" : ""}`}>
                          {m.text}
                        </p>
                      </div>
                    </div>
                ...
                {/* --- START: UNIFIED RESPONSIVE INPUT AREA --- */}
                <div className={`relative px-2 pb-2 pt-4 sm:px-4 sm:pb-4 sm:pt-4 border-t flex flex-col gap-2 transition-colors ${
                  isDarkMode ? "bg-zinc-800 border-zinc-700" : "bg-white border-zinc-100"
                }`}>
                  ...
                  {/* Input and Send Button (Always together) */}
                  <div className="flex gap-2">
                    <input
                      ...
                      className={`flex-1 p-3 lg:p-4 rounded-2xl outline-none focus:ring-2 focus:ring-sky-400 text-sm lg:text-base disabled:opacity-50 transition-colors ${
                        isDarkMode ? "bg-zinc-700 text-zinc-100 placeholder-zinc-400" : "bg-zinc-50 text-zinc-800"
                      }`}
                    />
                  ...
                  {/* Voice Tools (Stacked on mobile, row on desktop) */}
                  <div className="grid grid-cols-2 sm:flex sm:gap-2 gap-2">
                    ...
                    <div className={`relative flex items-center gap-1 p-1 rounded-2xl border shadow-inner transition-colors col-span-1 ${
                      isDarkMode ? "bg-zinc-700 border-zinc-600" : "bg-zinc-100 border-zinc-200"
                    }`}>
                      <button onClick={() => setSttDropdownOpen(p => !p)}
                        className={`flex-1 flex items-center justify-between py-2 pl-3 pr-1 text-xs lg:text-sm font-bold cursor-pointer ${isDarkMode ? "text-zinc-300" : "text-zinc-600"}`}>
                      ...
                      {sttDropdownOpen && (
                        <div className={`absolute bottom-full left-0 mb-1 border rounded-xl shadow-lg overflow-hidden z-50 min-w-full ${
                          isDarkMode ? "bg-zinc-800 border-zinc-600" : "bg-white border-zinc-200"
                        }`}>
                          {filteredSttLangs.map(l => (
                            <button key={l.code} onClick={() => { setSttLanguage(l.code); setSttDropdownOpen(false); }}
                              className={`w-full px-3 py-2 text-xs font-bold text-left whitespace-nowrap transition-colors ${
                                sttLanguage === l.code ? "text-sky-500 bg-sky-50" : isDarkMode ? "text-zinc-300 hover:bg-zinc-700" : "text-zinc-600 hover:bg-sky-50"
                              }`}>
                              {l.code.split("-")[1]}
                            </button>
                          ))}
                    ...
                    <button onClick={() => setShowMobileMenu(true)} disabled={isReconnecting}
                      className={`p-3 rounded-2xl transition-all cursor-pointer disabled:opacity-50 sm:hidden col-span-2 flex items-center justify-center ${
                ...
                {/* --- END: UNIFIED RESPONSIVE INPUT AREA --- */}
              </div>

              <div className={`hidden md:flex w-[35%] flex-col p-4 lg:p-5 gap-3 lg:gap-4 transition-colors ${
                isDarkMode ? "bg-zinc-800 border-zinc-700" : "bg-white border-zinc-100"
              }`}>
                <p className={`text-xs lg:text-sm font-bold uppercase tracking-wider ${isDarkMode ? "text-zinc-500" : "text-zinc-400"}`}>
                  {tr("classroom", "imagePanelTitle")}
                </p>

                <div
                  onClick={() => imageInputRef.current?.click()}
                  className={`flex-1 rounded-[1.5rem] border-2 border-dashed flex items-center justify-center overflow-hidden transition-all cursor-pointer ${
                    isDarkMode
                      ? "border-sky-700 bg-sky-900/20 hover:border-sky-500 hover:bg-sky-900/30"
                ...
                >
                  {selectedImage ? (
                    ...
                    <div className={`text-center ${isDarkMode ? "text-sky-400" : "text-sky-500"}`}>
                ...
                <button
                  ...
                  disabled={!imageReady || isReconnecting}
                  className={`w-full flex items-center justify-center gap-2 p-3 md:p-4 rounded-2xl font-bold text-sm lg:text-base transition-all cursor-pointer disabled:opacity-50 ${
                    imageReady
                      ...
                      : isDarkMode ? "bg-zinc-700 text-zinc-500 cursor-not-allowed" : "bg-zinc-100 text-zinc-300 cursor-not-allowed"
                  }`}
                >
                  <Send size={18} />
                  {tr("classroom", "sendImageBtn")}
                </button>

                <div className={`flex flex-col gap-2 pt-2 border-t ${isDarkMode ? "border-zinc-700" : "border-zinc-100"}`}>
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-1 p-3 rounded-2xl font-bold text-xs lg:text-sm transition-all cursor-pointer disabled:opacity-50 ${
                        isDarkMode
                          ? "bg-emerald-900/30 border-2 border-emerald-700 text-emerald-400 hover:bg-emerald-900/50"
                    ...
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-1 p-3 rounded-2xl font-bold text-xs lg:text-sm transition-all cursor-pointer disabled:opacity-50 ${
                        isDarkMode
                          ? "bg-sky-900/30 border-2 border-sky-700 text-sky-400 hover:bg-sky-900/50"
              ...
              <div className={`md:hidden w-full border-t flex flex-col p-3 gap-2 transition-colors ${
                isDarkMode ? "bg-zinc-800 border-zinc-700" : "bg-white border-zinc-100"
              }`}>
                <div
                  onClick={() => imageInputRef.current?.click()}
        ...
        {showMobileMenu && (
          ...
          >
            <motion.div
              ...
              className={`absolute bottom-0 left-0 right-0 rounded-t-[2rem] shadow-2xl p-6 ${isDarkMode ? "bg-zinc-800" : "bg-white"}`}
            >
              <div className="flex flex-col gap-3">
                <h3 className={`text-lg font-black mb-2 ${isDarkMode ? "text-zinc-100" : "text-zinc-800"}`}>Session Menu</h3>
                <button
                  onClick={handleViewReport}
                  className={`w-full flex items-center justify-center gap-2 p-4 rounded-2xl font-bold text-sm cursor-pointer ${
                    isDarkMode ? "bg-emerald-900/30 border-2 border-emerald-700 text-emerald-400" : "bg-emerald-50 border-2 border-emerald-200 text-emerald-700"
                  }`}
                >
                  <FileText size={20} /> View Session Report
                </button>
                <button
                  onClick={() => { setShowMobileMenu(false); setShowFlashcardsModal(true); }}
                  className={`w-full flex items-center justify-center gap-2 p-4 rounded-2xl font-bold text-sm cursor-pointer ${
                    isDarkMode ? "bg-sky-900/30 border-2 border-sky-700 text-sky-400" : "bg-sky-50 border-2 border-sky-200 text-sky-700"
                  }`}
                >
                  📚 Flashcards
                </button>
                <button
                  onClick={() => { setShowMobileMenu(false); setShowEndModal(true); }}
                  className={`w-full flex items-center justify-center gap-2 p-4 rounded-2xl font-bold text-sm cursor-pointer ${
                    isDarkMode ? "bg-red-900/30 border-2 border-red-700 text-red-400" : "bg-red-50 border-2 border-red-200 text-red-700"
                  }`}
                >
                ...
                <button
                  onClick={() => setShowMobileMenu(false)}
                  className={`w-full p-4 rounded-2xl font-bold text-sm cursor-pointer ${isDarkMode ? "bg-zinc-700 text-zinc-300" : "bg-zinc-100 text-zinc-600"}`}
                >
                  Cancel
                </button>
              </div>
      ...
      {showEndModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <motion.div
            ...
            className={`w-[90vw] sm:w-80 mx-4 rounded-[2rem] shadow-2xl border overflow-hidden ${
              isDarkMode ? "bg-zinc-800 border-zinc-700" : "bg-white border-zinc-100"
            }`}
          >
            <div className="px-6 py-5 flex flex-col gap-4">
              <div className="text-center">
                ...
                <h3 className={`font-black text-lg ${isDarkMode ? "text-zinc-100" : "text-zinc-800"}`}>End Session?</h3>
                <p className={`text-sm mt-1 ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}>Are you sure you want to exit?</p>
              </div>
              <div className="flex flex-col gap-2">
                <button onClick={handleEndSessionConfirm}
                  className="w-full bg-red-500 hover:bg-red-600 text-white font-black py-4 rounded-[1.5rem] transition-colors cursor-pointer">
                  End Session
                </button>
                <button onClick={() => setShowEndModal(false)}
                  className={`w-full font-bold py-3 rounded-[1.5rem] transition-colors cursor-pointer ${
          ...
          >
            <svg width="140" height="60" viewBox="0 0 140 60" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <g transform="translate(102, 4)">
                ...
                <circle cx="11" cy="17" r="4.5" fill="white" />
            ...
            <div className="flex flex-col gap-2 items-center">
              <p className={`text-xs text-center ${isDarkMode ? "text-zinc-300" : "text-zinc-600"}`}>{tr("messages", "analyzing")}</p>
              <p className={`text-xs text-center ${isDarkMode ? "text-zinc-500" : "text-zinc-400"}`}>{tr("messages", "analyzing", motherLang)}</p>
            </div>
          </motion.div>
        </div>
      )}
      ...
      {showNoFilesModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <motion.div
            ...
            className={`w-[90vw] sm:w-80 mx-4 rounded-[2rem] shadow-2xl border overflow-hidden transition-colors ${
              isDarkMode ? "bg-zinc-800 border-zinc-700" : "bg-white border-zinc-100"
            }`}
          >
            <div className="px-6 py-5 flex flex-col gap-4">
              <div className="text-center">
                <div className="text-3xl mb-2">📂</div>
                <h3 className={`font-black text-base ${isDarkMode ? "text-zinc-100" : "text-zinc-800"}`}>{tr("modals", "noFilesTitle")}</h3>
                <p className={`text-xs mt-1 leading-relaxed ${isDarkMode ? "text-zinc-400" : "text-zinc-400"}`}>{tr("modals", "noFilesSub")}</p>
              </div>
              <div className="flex flex-col gap-2">
                ...
                <button onClick={() => setShowNoFilesModal(false)}
                  className={`w-full font-bold py-3 rounded-[1.5rem] transition-colors cursor-pointer text-sm ${
                    isDarkMode ? "bg-zinc-700 hover:bg-zinc-600 text-zinc-300" : "bg-zinc-100 hover:bg-zinc-200 text-zinc-600"
                  }`}>
      ...
      {showLangModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <motion.div
            initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            className={`w-full max-w-sm mx-4 rounded-[2rem] shadow-2xl border overflow-hidden transition-colors ${
              isDarkMode ? "bg-zinc-800 border-zinc-700" : "bg-white border-zinc-100"
            }`}
          >
            <div className={`border-b px-6 py-4 flex items-center justify-between transition-colors ${
              isDarkMode ? "bg-zinc-700 border-zinc-600" : "bg-sky-50 border-sky-100"
            }`}>
              <h2 className={`font-black text-lg ${isDarkMode ? "text-zinc-100" : "text-zinc-800"}`}>{tr("modals", "langModalTitle")}</h2>
              <button onClick={() => setShowLangModal(false)} className={`transition-colors cursor-pointer ${isDarkMode ? "text-zinc-400 hover:text-zinc-200" : "text-zinc-400 hover:text-zinc-600"}`}>
                <X size={18} />
              </button>
            </div>
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { label: tr("modals", "motherLang"),  value: tmpMother, onChange: setTmpMother },
                ].map(({ label, value, onChange }) => (
                  <div key={label} className="space-y-2">
                    <label className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}>{label}</label>
                    <select value={value} onChange={e => onChange(e.target.value)}
                      className={`w-full rounded-xl border-2 p-3 text-sm font-bold outline-none focus:border-sky-400 cursor-pointer transition-colors ${
                        isDarkMode ? "border-zinc-600 bg-zinc-700 text-zinc-100" : "border-zinc-100 bg-zinc-50 text-zinc-700"
                      }`}>
              ...
              <div className="space-y-2">
                <label className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}>{tr("modals", "learningMode")}</label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    { key: "immersion", icon: "🔥", label: tr("modes", "immersion"), sub: tr("modeSubs", "immersion") },
                  ].map(m => (
                    <button key={m.key} onClick={() => setTmpMode(m.key)}
                      className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer text-center ${
                        tmpMode === m.key
                          ? "border-sky-400 bg-sky-50 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400"
                          : isDarkMode ? "border-zinc-600 bg-zinc-700 text-zinc-400 hover:border-zinc-500" : "border-zinc-100 bg-zinc-50 text-zinc-500 hover:border-zinc-300"
                      }`}>
              ...
              <button onClick={handleSaveLang}
                className="w-full bg-sky-500 hover:bg-sky-600 text-white font-black py-4 rounded-[1.5rem] transition-colors cursor-pointer shadow-lg shadow-sky-200/50">
                💾 {tr("modals", "saveBtn")}
              </button>
            </div>
          </motion.div>
      ...
      {showModeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <motion.div
            initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            className={`w-full max-w-md mx-4 rounded-[2rem] shadow-2xl border overflow-hidden transition-colors ${
              isDarkMode ? "bg-zinc-800 border-zinc-700" : "bg-white border-zinc-100"
            }`}
          ...
          >
            <div className={`border-b px-6 py-4 flex items-center justify-between transition-colors ${
              ...
              <h2 className={`font-black text-lg ${isDarkMode ? "text-zinc-100" : "text-zinc-800"}`}>{tr("modals", "modeModalTitle")}</h2>
              <button onClick={() => setShowModeModal(false)} className={`transition-colors cursor-pointer ${isDarkMode ? "text-zinc-400 hover:text-zinc-200" : "text-zinc-400 hover:text-zinc-600"}`}>
                <X size={18} />
              </button>
            </div>
            <div className="p-4 space-y-4 overflow-y-auto" style={{ maxHeight: "calc(85vh - 80px)" }}>
              {MODE_CATEGORIES.map(cat => (
                <div key={cat.key} className="space-y-2">
                  <h3 className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}>
                      ...
                      <button key={modeKey} onClick={() => handleModeSelect(modeKey)}
                        className={`w-full text-left p-3 rounded-xl border-2 transition-all cursor-pointer ${
                          selectedMode === modeKey
                            ...
                            : isDarkMode ? "border-zinc-600 bg-zinc-700 hover:border-zinc-500" : "border-zinc-100 bg-zinc-50 hover:border-zinc-300"
                        }`}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="text-lg">{MODE_ICONS[modeKey]}</span>
                            <div>
                              <div className={`text-sm font-black ${selectedMode === modeKey ? "text-sky-600 dark:text-sky-400" : isDarkMode ? "text-zinc-100" : "text-zinc-800"}`}>
                                {tr("modes", modeKey)}
                              </div>
                              <div className={`text-xs ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}>
                                {tr("modes", `${modeKey}Desc`)}
                              </div>
                            </div>
                          </div>
      ...
      {showFlashcardsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <motion.div
            initial={{ opacity: 0, scale: 0.92, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className={`w-full max-w-md mx-4 rounded-[2rem] shadow-2xl border overflow-hidden transition-colors flex flex-col ${
              isDarkMode ? "bg-zinc-800 border-zinc-700" : "bg-white border-zinc-100"
            }`}
          ...
          >
            <div className={`border-b px-6 py-4 flex items-center justify-between shrink-0 ${
              ...
              <div className="flex items-center gap-2">
                ...
                <h2 className={`font-black text-lg ${isDarkMode ? "text-zinc-100" : "text-zinc-800"}`}>Flashcards</h2>
              ...
              <button onClick={() => setShowFlashcardsModal(false)}
                className={`transition-colors cursor-pointer ${isDarkMode ? "text-zinc-400 hover:text-zinc-200" : "text-zinc-400 hover:text-zinc-600"}`}>
                <X size={18} />
              </button>
            </div>

            <div className={`flex-1 overflow-y-auto p-4 space-y-2 ${isDarkMode ? "bg-zinc-800" : "bg-white"}`}>
              {!user ? (
                <div className="flex flex-col items-center justify-center py-10 gap-3">
                  <span className="text-3xl">🔑</span>
                  <p className={`text-sm font-bold ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}>Sign in to save flashcards</p>
                  <button onClick={handleGoogleLogin}
                    className="bg-sky-500 hover:bg-sky-600 text-white font-black px-6 py-3 rounded-2xl text-sm cursor-pointer">
                    Sign in with Google
                  </button>
                </div>
              ) : flashcards.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 gap-2">
                  <span className="text-3xl">📭</span>
                  <p className={`text-sm font-bold ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}>No flashcards yet</p>
                  <p className={`text-xs ${isDarkMode ? "text-zinc-500" : "text-zinc-400"}`}>Words will be saved automatically during the session</p>
                </div>
              ) : (
                flashcards.map((card) => (
                  <div key={card.id} className={`flex items-center gap-2 p-3 rounded-xl border transition-colors ${
                    isDarkMode ? "bg-zinc-700 border-zinc-600" : "bg-zinc-50 border-zinc-100"
                  }`}>
                    {editingId === card.id ? (
                      <>
                        <div className="flex-1 flex flex-col gap-1">
                          <input
                            ...
                            placeholder="Word"
                            className={`px-2 py-1 rounded-lg text-sm outline-none border-2 focus:border-sky-400 ${
                              isDarkMode ? "bg-zinc-600 text-zinc-100 border-zinc-500" : "bg-white text-zinc-800 border-zinc-200"
                            }`}
                            autoFocus
                          />
                          <input
                        ...
                        <button onClick={() => handleUpdateFlashcard(card.id)}
                          className="text-emerald-500 hover:text-emerald-600 cursor-pointer font-black text-sm px-2">✓</button>
                        <button onClick={() => { setEditingId(null); setEditingWord(""); setEditingTranslation(""); }}
                          className="text-zinc-400 hover:text-zinc-600 cursor-pointer text-sm px-1">✕</button>
                      </>
                    ) : (
                      <>
                        <div className="flex-1 flex items-center gap-2">
                          ...
                          <span className={`text-sm font-bold ${isDarkMode ? "text-zinc-100" : "text-zinc-800"}`}>
                            {card.word}
                          </span>
                          <span className={`text-xs ${isDarkMode ? "text-zinc-500" : "text-zinc-400"}`}>→</span>
                          <span className={`text-xs font-bold px-1 rounded ${
                            isDarkMode ? "bg-emerald-900 text-emerald-400" : "bg-emerald-100 text-emerald-600"
                          }`}>
                            {card.motherLang.toUpperCase()}:
                          </span>
                        ...
                        <button onClick={() => { setEditingId(card.id); setEditingWord(card.word); setEditingTranslation(card.translation); }}
                          className={`text-xs px-2 py-1 rounded-lg cursor-pointer transition-colors ${
                            isDarkMode ? "text-zinc-400 hover:text-zinc-200" : "text-zinc-400 hover:text-zinc-600"
                          }`}>✏️</button>
                        <button onClick={() => handleDeleteFlashcard(card.id)}
                          className="text-red-400 hover:text-red-600 cursor-pointer text-sm px-1">✕</button>
                      </>
                    )}
                  </div>
                ))
